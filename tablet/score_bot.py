...
class ScoreBot():
    ...
    def moveForward(self):
        if (time.time() - self.stateStartTime > 3):
            self.driver.stopMotors()
            if (self.atReactor):
                if self.sensorManager.vision.seeReactor() and abs(self.sensorManager.vision.goalReactor[0]) < 12 and self.sensorManager.vision.goalReactor[1] < 60 and self.sensorManager.vision.goalReactor[2] > 280:
